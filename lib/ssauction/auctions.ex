defmodule SSAuction.Auctions do
  ...
  def delete_auction(%Auction{} = auction) do
    ...
    Repo.delete_all(
      from r in "auctions_users",
        where: r.auction_id == ^auction.id,
        select: [r.id, r.auction_id, r.user_id]
    )
  ...
  def dedup_years(%Auction{} = auction) do
    ...
    case Regex.named_captures(
           ~r/(?<year1>\d{4})-(?<year2>\d{4})-(?<league>\w\w)/,
           years_and_league
         ) do
  ...
  def list_teams_with_ssnum(auction = %Auction{}) do
    Repo.all(
      from t in Team,
        where: t.auction_id == ^auction.id and not is_nil(t.ssnum),
        order_by: t.ssnum
    )
  end
  ...
  def get_rostered_players_with_rostered_at(%Auction{} = auction, %{
        sort_by: sort_by,
  ...
  def get_rostered_players_with_rostered_at_and_surplus(%Auction{} = auction, %Team{} = team, %{
        sort_by: sort_by,
        sort_order: sort_order
      }) do
    sort_order = if sort_by == :rostered_at, do: {sort_order, DateTime}, else: sort_order
  ...
  def get_cut_players_with_cut_at_and_cost(%Auction{} = auction, %{
        sort_by: sort_by,
        sort_order: sort_order
      }) do
  ...
  def get_teams(%Auction{} = auction) do
    ...
    Repo.preload(auction, [:teams]).teams
    |> Enum.map(fn team ->
      seconds_until_new_nominations_open = DateTime.diff(team.new_nominations_open_at, now)
  ...
  def create_team(%Auction{} = auction,
        name: team_name,
        new_nominations_open_at: new_nominations_open_at
      ) do
    team =
      Ecto.build_assoc(auction, :teams, %{
        name: team_name,
        new_nominations_open_at: new_nominations_open_at,
  ...
  defp get_auction_user_ids(%Auction{} = auction) do
    Enum.sort(
      Enum.map(
        Enum.concat(
          Enum.map(Repo.preload(auction, [:teams]).teams, fn t ->
            Repo.preload(t, [:users]).users
          end)
        ),
        fn u -> u.id end
      )
  ...
  def check_for_expired_bids(auction_id) do
    ...
    open_bids =
      from b in subquery(auction_bids),
        where: not b.closed
  ...
  def stop_nominations(auction = %Auction{}, reset_nominations_open_at_date) do
    Repo.preload(auction, [:teams]).teams
    |> Enum.map(fn team ->
      Teams.set_unused_nominations(team, 0)
  ...
  def check_for_expired_nominations(team = %Team{}, auction = %Auction{}) do
    ...
    if team.time_nominations_expire != nil and
         DateTime.diff(now, team.time_nominations_expire) >= 0 do
  ...
  defp auto_nominate(team = %Team{}, auction = %Auction{}) do
    if team.unused_nominations > 0 do
      for _ <- 1..team.unused_nominations do
        if Teams.has_open_roster_spot?(team, auction) and
             Teams.dollars_remaining_for_bids_including_hidden(team) > 0 do
          cond do
      ...
      team
      |> Team.changeset(%{time_nominations_expire: nil, unused_nominations: 0})
      |> Repo.update()
